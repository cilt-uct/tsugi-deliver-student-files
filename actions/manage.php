...
class UploadHandler {
    ...
    public function __construct($pdo, $p, $link_id, $user_id,
                                $instructor, $EID,
        ...
        $this->options = array(
            ...
            'accept_file_types' => '/\.(?:pdf|docx?|xlsx?|pptx?|txt|rtf|csv|jpg|jpeg|png|gif|bmp)$/i',
    ...
    protected function get_download_url($blob_id, $version = null, $direct = false) {
        if (!$direct && $this->options['download_via_php']) {
            $url = $this->options['script_url']
                .$this->get_query_separator($this->options['script_url'])
                .$this->get_singular_param_name()
                .'='.rawurlencode($blob_id);
            if ($version) {
    ...
    protected function set_additional_file_properties($file) {
        $file->deleteUrl = $this->options['script_url']
            .$this->get_query_separator($this->options['script_url'])
            .$this->get_singular_param_name()
            .'='.rawurlencode($file->blob_id);
        $file->deleteType = $this->options['delete_type'];
        if ($file->deleteType !== 'DELETE') {
            $file->deleteUrl .= '&_method=DELETE';
        }
        $file->deleteUrl = addSession($file->deleteUrl);
    ...
    protected function get_file_object($src) {

        $file = new \stdClass();
        $file->name = $src['name'];
        $file->blob_id = $src['blob_id'];
        $file->type = $src['type'];
        $file->size = $this->get_file_size($src['full_path']);
        $file->url  = addSession($this->get_download_url($src['blob_id']));
    ...
    protected function get_file_objects($iteration_method = 'get_file_object') {
        ...
        $sql = "SELECT `A`.blob_id, `blob`.file_name as `name`, `blob`.contenttype as `type`, `blob`.`path` as `full_path` " .
                "FROM {$this->p}student_files `A` ".
                "LEFT JOIN {$this->p}blob_file `blob` on `blob`.file_id = `A`.blob_id and `blob`.link_id = `A`.link_id ".
                "WHERE `A`.link_id=:linkId and `blob`.file_name is not null";
        $sql_ar = array(":linkId" => $this->link_id);

        // limit files based on EID because user is not instructor
        if (!$this->instructor) {
            $sql .= ' and `blob`.file_name like :wild_name';
            $sql_ar[':wild_name'] = '%'. $this->EID .'%';
        }

        // return ['sql' => $sql, 'sql_ar' => $sql_ar];
    ...
    protected function handle_file_upload($uploaded_file, $name, $size, $type, $error, $index = null, $content_range = null) {
        ...
        if ($this->validate($uploaded_file, $file, $error, $index, $content_range)) {
            ...
            if ($safety1 !== true) {
                $file->error = "Error: " . $safety1;
                return $file;
            }

            $blob_id = BlobUtil::uploadToBlob($construct);
            if ($blob_id === false) {
                $file->error = 'Problem storing file in server: ' . $file->name;
                return $file;
            }

            $file->blob_id = $blob_id;
            $file->url = addSession($this->get_download_url($blob_id));

            // Save success so add info to gallery database
            $newStmt = $this->pdo ->prepare("INSERT INTO {$this->p}student_files (link_id, user_id, blob_id) values (:link_id, :userId, :blobId)");
            $newStmt->execute(array(":link_id" => $this->link_id, ":userId" =>$this->user_id, ":blobId" => $blob_id));
    ...
    protected function get_file_names_params() {
        ...
        foreach ($params as $key => $value) {
            $params[$key] = $this->basename((string) stripslashes($value));
        }
        return $params;
    }
    ...
    protected function download() {
        ...
        $blob_id = $this->get_file_name_param();
        if ($blob_id) {

            $sql = "SELECT `A`.blob_id, `blob`.file_name as `name`, `blob`.contenttype as `type`, `blob`.`path` as `full_path` " .
                    "FROM {$this->p}student_files `A` ".
                    "LEFT JOIN {$this->p}blob_file `blob` on `blob`.file_id = `A`.blob_id and `blob`.link_id = `A`.link_id ".
                    "WHERE `A`.link_id=:linkId and `A`.blob_id=:blob_id";
            $sql_ar = array(":linkId" => $this->link_id, ":blob_id" => $blob_id);
    ...
    public function get($print_response = true) {
        ...
        if ($blob_id) {
            ...
            $stmt = $this->pdo->prepare("SELECT `A`.blob_id, `blob`.file_name as `name`, `blob`.contenttype as `type`, `blob`.`path` as `full_path` " .
                "FROM {$this->p}student_files `A` ".
                "LEFT JOIN {$this->p}blob_file `blob` on `blob`.file_id = `A`.blob_id and `blob`.link_id = `A`.link_id ".
                "WHERE `A`.link_id=:linkId and `A`.blob_id=:blob_id");
            $stmt->execute(array(":linkId" => $this->link_id, ":blob_id" => $blob_id));
            $file = $stmt->fetch(PDO::FETCH_ASSOC);

            $response = array(
                $file['name'] => $this->get_file_object($file)
            );
        } else {
            $response = array(
    ...
    public function delete($print_response = true) {
        $blob_ids = $this->get_file_names_params();

        if (empty($blob_ids)) {
            $blob_ids = array($this->get_file_name_param());
        }
        ...
        foreach ($blob_ids as $blob_id) {
            ...
            $linkStmt = $this->pdo->prepare("delete from {$this->p}student_files where blob_id = :blob_id");
            $linkStmt->execute(array(":blob_id" => $blob_id));

            $verifyStmt = $this->pdo->prepare("select file_id from {$this->p}blob_file where file_id = :blob_id");
            $verifyStmt->execute(array(":blob_id" => $blob_id));

            $response[$blob_id] = ($linkStmt->rowCount() > 0) && ($verifyStmt->rowCount() == 0);
        }
        return $this->generate_response($response, $print_response);
    }
...
$upload_max_size_bytes = BlobUtil::maxUploadBytes();
$options = [
    'max_file_size' => $upload_max_size_bytes
];
$upload_handler = new UploadHandler($PDOX, $p, $LINK->id, $USER->id,
                                    $USER->instructor, $EID,
                                    $options);
